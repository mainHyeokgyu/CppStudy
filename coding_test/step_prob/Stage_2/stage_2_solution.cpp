...
// void SettingAlarm(int hour, int minute);
// int main(void)
// {
//     int hour(0), minute(0);
//     cin>>hour>>minute;
//     SettingAlarm(hour,minute);
...
//        인공지능 오븐을 개발하려고 한다. 인공지능 오븐을 사용하는 방법은 적당한
//        양의 오리 훈제 재료를 인공지능 오븐에 넣으면 된다. 그러면 인공지능 오븐은
//        오븐구이가 끝나는 시간을 분 단위로 자동적으로 계산한다. 
//        또한, KOI 전자의 인공지능 오븐 앞면에는 사용자에게 훈제오리구이 요리가 끝나는 시각을
//        알려 주는 디지털 시계가 있다. 
//        훈제오리구이를 시작하는 시각과 오븐구이를 하는 데 필요한 시간이 분단위로 주어졌을 때,
//        오븐구이가 끝나는 시각을 계산하는 프로그램을 작성하시오.
// 입력 : 첫째 줄에는 현재 시각이 나온다. 현재 시각은 시 A (0 ≤ A ≤ 23) 와 분 B (0 ≤ B ≤ 59)가
...
// void end_time(int hour, int minute, int during_minute);
// int main(void)
// {
//     int hour(0), minute(0), during_minute(0);
//     cin>>hour>>minute;
//     cin>>during_minute;
//     end_time(hour,minute,during_minute);

//     return 0;
// }
// void end_time(int hour, int minute, int during_minute)
// {
//     int during_hour(during_minute/60);
//     during_minute%=60;
//     minute+=during_minute;
//     hour+=during_hour;
...
// 문제 : 1에서부터 6까지의 눈을 가진 3개의 주사위를 던져서
...
void dice_reward(int dice_1, int dice_2, int dice_3);
int main(void)
{
    int dice_1(0), dice_2(0), dice_3(0);
    cin>>dice_1>>dice_2>>dice_3;
    dice_reward(dice_1, dice_2, dice_3);
...
void dice_reward(int dice_1, int dice_2, int dice_3)
{
    ...
    {
        cout<<max({dice_1,dice_2,dice_3})*100;
    }
}